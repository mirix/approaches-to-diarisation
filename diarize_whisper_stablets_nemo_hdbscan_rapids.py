...
for wavs in os.scandir('diarealsamples'):
	...
	if wavs.is_file() and wavs.path.endswith('.wav'):
		...
		if len(long_seg) < 10 and len(df_transcript) >= 2:
			...
			try:
				clusterable_embedding = cuml.manifold.UMAP(
			...
			try:
				clusterable_embedding_large = cuml.manifold.UMAP(
				    n_neighbors=n_neighbors,
				    min_dist=.0,
				    n_components=comp,
				    random_state=31416,
		...
		if len(list(set(clustered))) > 1:
			print('Cluster persistence (1 is perfect): ' + str(clusterer.cluster_persistence_.astype(float).round(2).to_dict('records')[0]))
		print(' ')
